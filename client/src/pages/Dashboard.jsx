...
const Card = ({ label, count, bg, icon, daysSinceCreated, showDays }) => {
  return (
      ...
      <div className='h-full flex flex-1 flex-col justify-between'>
        ...
        {showDays ? (
          <>
            ...
            <span className='text-xl font-semibold'>{daysSinceCreated} days since</span>
          </>
        ) : (
          <>
            <span className='text-2xl font-semibold'>{count}</span>
...
const Dashboard = () => {
  ...
  if (user?.createdAt) {
    ...
    const now = new Date('2025-05-11T19:56:27+05:30');
    const diffTime = Math.abs(now - created);
    daysSinceCreated = Math.floor(diffTime / (1000 * 60 * 60 * 24));
  }
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-5'>
          {stats?.map(({ icon, bg, label, total }, index) => (
            <Card
              ...
              count={total}
              showDays={index === 0}
              daysSinceCreated={index === 0 ? daysSinceCreated : undefined}
            />
          ))}
        </div>
