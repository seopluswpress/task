    ...
    return (
      <>
        <Menu as='div' className='relative inline-block text-left'>
          ...
          <CustomTransition>
          <Menu.Items className='absolute p-4 right-0 bottom-full mb-2 w-56 origin-bottom-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-50'>
                    ...
                    {({ active }) => (
                      <button
                        ...
                        className={clsx(
                          ...
                          "group flex gap-2 w-full items-center rounded-md px-2 py-2 text-sm disabled:opacity-70"
                        )}
                      >
                        {el.icon}
                        {el.label}
                ...
                <div className='px-1 py-1 space-y-2'>
                  {items.map((el, index) => (
                    <Menu.Item key={el.label}>
                      {({ active }) => (
                        <button
                          disabled={index === 0 ? false : index === 1 ? !user.isAdmin : index === 2 ? false : !user.isAdmin}
                          onClick={el?.onClick}
                          className={`${
